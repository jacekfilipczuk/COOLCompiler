...
class A {
   ...
   method1(num1 : Int, num2 : Int) : B {  -- plus
      (let x : Int in
	 {
            x <- num1 + num2;
	    (new B).set_var(x);
...
*)
	...
	if s.substr(0,1) = "-" then ~a2i_aux(s.substr(1,s.length()-1)) else
        if s.substr(0,1,5) = "+" then a2i_aux(s.substr(1,s.length()-1)) else
           a2i_aux(s)
        fi fi fi
     };
   ...
   main() : Object {
      {
         ...
         while flag loop
	       ...
	       char <- menu();
                  ...
                  if char = "d" then avar <- (new C)@TypeSbagliato.method5(avar.value()) else
		          -- factorial
